...
router.post('/', async (req, res) => {
  ...
  router.put('/:id', async (req, res) => {
      try {
          const { id } = req.params;
          const [updated] = await newTransactionModel.update(req.body, { where: { id } });
          if (updated) {
              const updateTransaction = await newTransactionModel.findByPk(req.params.id);
  ...
  router.delete('/:id', async (req, res) => {
      try {
          const { id } = req.params;
          const deleteTransaction = await newTransactionModel.destroy({ where: { id } });
